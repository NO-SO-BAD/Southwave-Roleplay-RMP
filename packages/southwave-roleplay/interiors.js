...
mp.events.addCommand('interior', function (player, _, interiorName) {
    if (!player || player.id === -1)
        return;
    var interior = INTERIORS[interiorName];
    if (!interior) {
    ...
    player.outputChatBox("!{#00ff00}Teleportado a ".concat(interior.name));
    player.outputChatBox('!{#ffaa00}Usa /exit para salir');
});
mp.events.addCommand('exit', function (player) {
